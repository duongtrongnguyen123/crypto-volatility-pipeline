...
\title{\textbf{BÁO CÁO MÔN HỌC DỮ LIỆU LỚN (BIG DATA)}\\ \vspace{1cm} \huge \textbf{Suy luận Quan hệ và Thời gian của Mô hình Ngôn ngữ Lớn để Dự đoán Sụp đổ Giá Cổ phiếu và Crypto}\\ \vspace{0.5cm} \large (Temporal Relational Reasoning of LLMs for Stock Crash Prediction)}
\author{\vspace{1cm}\\
...
\pagenumbering{roman}
\begin{abstract}
Báo cáo này trình bày toàn diện về việc xây dựng một hệ thống \textbf{dự đoán rủi ro sụp đổ (crash) của một danh mục cổ phiếu vốn hóa lớn và thị trường tiền mã hóa (crypto)}. Dựa trên framework Temporal Relational Reasoning (TRR) \cite{trr_paper}, hệ thống tận dụng \textbf{Mô hình Ngôn ngữ Lớn (LLM)} để đọc, phân tích và suy luận trực tiếp trên các bản tin tài chính bằng phương pháp \textbf{zero-shot} mà không cần huấn luyện lại trọng số mô hình. Chúng tôi thực hiện quy trình 4 pha: \textbf{Brainstorm $\rightarrow$ Memory $\rightarrow$ Attention $\rightarrow$ Reason}, tích hợp kỹ thuật Retrieval-Augmented Generation (RAG) \cite{rag} và đánh giá trên các hệ thống Big Data phân tán.
...
\begin{enumerate}
    \item \textbf{Cải thiện RAG có ý nghĩa toàn diện:} Trên mẫu lớn 8 năm (78 đợt sụp đổ), RAG cải thiện hiệu năng từ 0.615 lên 0.652, với $p=0.047$. Điều này chứng minh RAG giúp ích thực sự khi cung cấp các sự kiện tiền lệ làm ngữ cảnh tham chiếu.
...
LLM (đặc biệt là phiên bản 32B) thường gặp hiện tượng \textit{quá tự tin (Overconfidence)}. Chúng thường đưa ra dự đoán rủi ro $60\%-80\%$ cho những ngày chỉ có tín hiệu tiêu cực nhẹ, trong khi tỷ lệ sụp đổ cơ sở chỉ là $11\%$. Brier Score ban đầu là 0.191 (cực kỳ tệ).

...
from webapp import live as _live_boot

st.set_page_config(page_title="TRR — Stock Crash Radar", page_icon="📉",
                   layout="wide", initial_sidebar_state="expanded")

# Pre-warm the local 7B summarizer in the background at startup, so the live
# news summary is LLM-generated by the time the Live tab is viewed (non-blocking;
# falls back to the rule-based template until the model is ready).
_live_boot._warm_summarizer_async()
...
with tab_live:
    ...
    def _live_panel():
        ...
        try:
            res = _live.live_news_summary(use_llm=True)
            sig = res["signals"]
